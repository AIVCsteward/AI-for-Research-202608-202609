...
def _resolve_train_mask(meta, train_mask=None):
    """Return an index-aligned train-only mask and reject ambiguous input."""
...
def build_matched_control_pairs(meta, train_mask=None, match_keys=MATCH_KEYS):
    """Build exact treatment-control pairs using training rows only.

    A treatment is retained only when an exact Water/DMSO control exists for
    every matching key.  No global-control fallback is allowed here because a
    fallback would turn an estimated baseline into a fabricated FC label.

    Returns a DataFrame with one row per matched treatment and a tuple of all
    matching control sample IDs.  Keeping every replicate lets downstream code
    average controls protein by protein instead of arbitrarily taking the first.
    """
...
def compute_fold_change(
    ...
    """Compute train-only treatment-minus-matched-control FC targets.

    Multiple matched controls are averaged separately for each protein using
    only observed values.  A protein contributes to ``fc_mask`` only when the
    treatment and at least one matched control are both observed.

    Returns a dictionary containing ``pairs``, ``fc_true`` and ``fc_mask``.
    ``fc_true`` uses NaN outside the valid mask so accidental unmasked use is
    easy to detect.
    """
    ...
    # Align once, then use integer row positions.  Repeated DataFrame ``.loc``
    # calls across thousands of proteins are unnecessarily expensive.
