"""
模型定义模块：条件编码 MLP → Encoder-Decoder 架构

当前实现（Phase 1 baseline → Phase 2 升级）:
  ConditionMLP: 2 隐藏层 MLP (256→256)，one-hot 条件特征 → 全蛋白组预测
  ConditionEncoder (B1): 3层MLP，输出 256 维条件嵌入
  CalibrationHead (B3): 轻量 MLP → 加性蛋白偏移（零初始化）
  AIVCModel: Encoder + Decoder + Calibration 完整模型
...
class ProteinGraphSmooth(nn.Module):
    ...
    def forward(self, y_t, edge_index):
        ...
        neighbor_sum = neighbor_sum.index_add(0, dst, y_t[src])
        neighbor_mean = neighbor_sum / deg  # (P, N)

        # 可学习平滑：向邻居均值偏移 α 比例
        y_smoothed = y_t + self.alpha * (neighbor_mean - y_t)
